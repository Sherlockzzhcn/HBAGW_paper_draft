...
\begin{authorresponse}
    ...
    \ZZH{Actually the posterior distributions for APR4 and SLy are not very largely overlapping in the corner plots. Even in 
    figure~\ref{fig:SLy}, the difference between the two EOSs is visible. It is the width of constraints that remains unchanged 
    for various EOSs. In this case, should I show results of the two EOSs in the same figure? Anyway I present the result as below}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\textwidth]{fig_comparison_corner_plot_EOS.pdf}
        \caption{Corner plots: APR4 vs SLy}
...
\begin{authorresponse}
    ...
    marginalizing out the orthogonal minor component. After such projection, the original two model parameters can be 
    expressed by $X$ as $a_2 = -0.9296 + 0.9848 X$ and $b_2 = 0.4504 + -0.1738 X$ and the linear model 
    becomes a dimension-reduced model with one parameter $X$. 
    ...
    \begin{revisedtext}
        ``For other parameterizations beyond polynomial models, our methodology is applicable to them as well.''
    \end{revisedtext}
\end{authorresponse}
