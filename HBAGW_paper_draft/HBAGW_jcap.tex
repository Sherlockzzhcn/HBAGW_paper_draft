...
\abstract{A universal ``Love-Q'' relation exists between 
the dimensionless tidal deformability $\Lambda$ and the 
dimensionless quadrupole moment $Q$ for neutron stars (NSs). 
However, this Love-Q 
relation has not yet been directly measured through observations. 
Gravitational waves (GWs) emitted from binary NS (BNS) coalescences 
provide an opportunity for such measurement of Love-Q relation, 
especially using future high-precision GW observations expected from 
next-generation ground-based detectors. 
In this study, within a hierarchical Bayesian framework,
we combine multiple simulated GW events to constrain Love-Q relation. 
We simulate 1000 GW sources and select 20 events with the highest SNRs and spins for the analysis.
By inspecting four parameterization models of Love-Q relation, 
we find that the inferred linear relation between $\ln\Lambda$ and $\ln Q$ 
is sufficient for placing constraints on Love-Q relation under the observational precision 
expected from next-generation GW detectors. 
Strong correlations between the parameters are found in all models. 
...
our results suggest that the characteristic length $\xi_{\mathrm{CS}}^{1/4}$ can be constrained 
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.8\textwidth]{fig_2d-5d_difference.pdf}% Here is how to import EPS art
...
\includegraphics[width=\textwidth]{fig_hierarchical_results_APR4_2d.pdf}
    ...
    Love-Q relation~\cite{Yagi_2017} in orange. The right panel shows how the
...
\centering
\includegraphics[width=0.8\linewidth]{fig_Hyper_parameter_5d.pdf}% Here is how to import EPS art
\end{minipage}
\hfill
\begin{minipage}[t]{0.49\textwidth}
