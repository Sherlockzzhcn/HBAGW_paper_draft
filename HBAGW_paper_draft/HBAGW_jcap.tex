...
\abstract{Theoretical computations have predicted a universal ``Love-Q'' relation 
between the dimensionless tidal deformability $\Lambda$ and the 
...
measurements, making it possible to conduct an experimental 
measurement of Love-Q relation with GW observations. 
In this study, we explore the prospects of constraining the Love-Q 
...
the inferred linear relation between $\ln\Lambda$ and $\ln Q$ 
is able to provide accurate constraints of the Love-Q relation with GWs. 
...
our results suggest that the characteristic length $\xi_{\mathrm{CS}}^{1/4}$ can be limited 
...
density of the NS. Yet the EOS of NSs has not been determined by current observations.

When it comes to testing gravity with NS observations, one can encounter a degeneracy
between uncertainties of both the
...
that are insensitive to EOS uncertainties but dependent on the underlying gravity theory.
Yagi and Yunes~\cite{Yagi:2013bca,Yagi:2013awa} first proposed such universal relation between 
...
On the other hand, the relation in modified gravity theories, 
such as dynamical Chern-Simons (dCS) gravity, can deviate from the one 
predicted by GR~\cite{Yagi_2017,Gupta:2017vsl,Yunes:2025xwp}. 
Therefore, an EOS-insensitive test of gravity can be conducted 
with the I-Love-Q relation. 
...
Taking the dCS gravity as an example, we find that the
...
{\Lambda}_i=\{\Lambda^{(i)}_{1},\Lambda^{(i)}_{2}\}$ and quadrupole moments $\bm{Q}_i=\{Q^{(i)}_
{1},Q^{(i)}_{2}\}$ of the two NSs of the $i\text{-th}$ event are treated as independent
...
\begin{table}[t]
    ...
    \sisetup{
        ...
        the results of least squares regression performed on 1000 points uniformly picked in logarithmic space from Yagi-Yunes relation. Priors are chosen to be uniform distributions, and the ranges
...
to figure~\ref{corner2-d}, the Yagi-Yunes Love-Q relation is covered by the 90\% credible region. 
